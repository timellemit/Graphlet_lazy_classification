...
sample_adress = os.path.join(input_address, "PTC_sample2_44x44x44")
...
for grouptype in ['MM', 'FR','FM', 'MR']:
    ...
    true_labels = select_labels(
            ...
            grouptype=grouptype)
    for k_nodes in [5]:
        subreport_lazy, subreport_svm, subreport_knn = np.array([]), np.array([]), np.array([])
...
#                         output_time=True)
#         
#         knn_pred, knn_pred_time = molecules.knn_graphlet_classify(test_dir, all_labels_filename, 
#                         grouptype=grouptype, 
...
#         subreport_knn = np.append(subreport_knn, 1)
#         subreport_knn = np.append(subreport_knn, k_nodes)
#         subreport_knn = np.append(subreport_knn, metrics.accuracy_score(true_labels, knn_pred))
#         # add precision, recall and F-score to the report
#         subreport_knn = np.append(subreport_knn, metrics.precision_recall_fscore_support(true_labels, knn_pred, average="macro")[:3])
#         subreport_knn = np.append(subreport_knn, knn_pred_time)
#         if len(group_report):
#             group_report = np.vstack((group_report, subreport_knn))
#         else:
#             group_report = subreport_knn
#                 
...
#         print "Lazy time: {0}, SVM time: {1}, KNN time: {2}".format(lazy_pred_time, svm_pred_time , knn_pred_time)
...
#         print(metrics.classification_report(true_labels, knn_pred))
#         print(metrics.confusion_matrix(true_labels, knn_pred))
...
print report
np.savetxt(os.path.join("C:\\Users\\User\\YandexDisk\\conferences_journals\\ECML_PKDD_2015", "report_44x44x44.txt"), report, fmt='%.2f',delimiter=',')